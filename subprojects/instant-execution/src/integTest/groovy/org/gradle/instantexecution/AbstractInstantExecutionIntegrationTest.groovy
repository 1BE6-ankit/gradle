...
class AbstractInstantExecutionIntegrationTest extends AbstractIntegrationSpec {
    ...
    protected void expectInstantExecutionFailure(
        ...
        // TODO assert problems are deduplicated in failure causes
        verifyInstantExecutionProblems(executer.workingDir, totalProblemsCount, uniqueProblems as List)
    ...
    protected void expectNoInstantExecutionProblem() {
        verifyInstantExecutionProblems(executer.workingDir, 0, [])
    }
    ...
    protected void expectInstantExecutionProblems(
        ...
        if (problems.length == 0) {
            throw new IllegalArgumentException("Use expectNoInstantExecutionProblem() when no instant execution problems are to be expected")
        }
        verifyInstantExecutionProblems(executer.workingDir, count, problems as List)
    }

    private void verifyInstantExecutionProblems(
        File rootDir = executer.workingDir,
        int totalProblemsCount,
        List<String> uniqueProblems
    ) {
        if (result instanceof ExecutionFailure) {
            assertProblemsFailure(totalProblemsCount, uniqueProblems)
        } else {
            assertProblemsConsoleSummary(totalProblemsCount, uniqueProblems)
        }
        assertProblemsHtmlReport(totalProblemsCount, uniqueProblems.size())
    }

    private void assertProblemsFailure(int totalProblemsCount, List<String> uniqueProblems) {
